...
namespace DbConnector.Example.Repositories
{
        ...
        where TDbConnection : DbConnection
    {
        ...
        {
            return _dbConnector.ReadToList<Employee>("SELECT * FROM Employees", new { x = 2 }).ExecuteHandledAsync();
        }

        public Task<List<Employee>> GetAllSimple()
        {
            return _dbConnector.ReadToList<Employee>("SELECT * FROM Employees").ExecuteAsync();
        }

        public Task<Employee> Get(int id)
        {
            return _dbConnector.ReadFirstOrDefault<Employee>("dbo.GetEmployee", new { id }, CommandType.StoredProcedure).ExecuteAsync();
        }

        public Task<DataTable> GetByDataTable()
        {
            return _dbConnector.ReadToDataTable("SELECT * FROM Employees").ExecuteAsync();
        }
        ...
        public Task<int?> Insert(Employee entity)
        {
            return _dbConnector.NonQuery(
                    ...
                    new { entity.Name, entity.ModifiedDate }).ExecuteAsync();
        }
    }
}
